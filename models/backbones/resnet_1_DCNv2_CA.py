...
class ResNet_1_DCNV2_CA(nn.Module):
    def __init__(self, model_name: str = '50') -> None:
        ...
        self.conv1 = DCNv2(3, self.inplanes, 7, 2, 3)
        self.offset1 = nn.Conv2d(3, self.inplanes, 1, bias=False)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x=self.conv1(x,self.offset1(x))

        x = self.maxpool(F.relu(self.bn1(x)))   # [1, 64, H/4, W/4]
        x=self.ca(x)
